...
def add_user(username,password):
		...
		if os.path.exists(cwd+"temp/.htpasswd") == False:
				ht = HtpasswdFile(cwd+"temp/.htpasswd", new=True)
				result = ht.set_password(username, password)
				ht.save()
				return result
		else:
				ht = HtpasswdFile(cwd+"temp/.htpasswd")
				result = ht.set_password(username, password)
				ht.save()
				if result == False:
						return True
...
def login(data):
		...
		if False:# result == ():
				...
				if check_user_password_htpasswd(roll_no,password):
						#s = requests.Session()
						#s.post('http://127.0.0.1/temp/student-login.html', data=login_data)
						return "True"
				else:
