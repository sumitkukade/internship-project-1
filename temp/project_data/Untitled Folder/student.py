    ...
    for i in range(0,len(appid)):
       ...
       if len(val)!=0:  
          ...
          '''if ststo=='ApplicationSubmitted':
             ...
             ss="""select status from  StudentStatusnew where Document=%s and RollNo=%s"""
             cursor.execute(ss,(appname[i],rollno));
             rs=cursor.fetchall()
             return rs
             if(len(rs)!=0):
                  ss="""update  StudentStatus set status=%s where Document=%s and RollNo=%s"""
                  cursor.execute(ss,(appname[i],rollno));'''
