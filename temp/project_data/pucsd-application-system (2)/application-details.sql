...
CREATE TABLE IF NOT EXISTS modificationInApplication (
  requestId bigInt NOT NULL,
  appId varchar(5) NOT NULL,
  userId varchar(15) NOT NULL,
  requestArrivalTime timestamp NOT NULL,
  ...
  params text,
  applicationPdf blob,
  Constraint pk_modificationInApplication Primary Key(requestId,appId),
  Foreign Key(requestId) References requestindex(requestId)
  ON DELETE RESTRICT ON UPDATE CASCADE,
  Foreign Key(appId) References applicationDomain(appId)
  ON DELETE RESTRICT ON UPDATE CASCADE,
  Foreign Key(userId) References studentDetailsDomain(rollNumber)
  ON DELETE RESTRICT ON UPDATE CASCADE
);
