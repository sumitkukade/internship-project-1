...
BEGIN
  ...
  declare ismodificationState integer;
  set count = 1;
  set cnt = 0;
  set flag = 0;
  ...
  if(len <= 5 and len >= 3) then
     while count<= len do
        ...
        set ismodificationState = (select string = (select applicationState from applicationProcess.modificationStateDomain order by applicationState desc limit 1));
        if(isAppId = 1) then
